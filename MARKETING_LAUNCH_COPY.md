# Pantry & Plate Launch Copy

## Positioning

Pantry & Plate helps you plan meals from what's already in your kitchen, organize the recipes you actually cook, and build smarter grocery lists around what's missing.

Core message:

- Know what you have.
- Plan what you'll cook.
- Shop only what's missing.

Store status:

- iOS: https://apps.apple.com/us/app/pantry-plate/id6761082174
- Google Play: https://play.google.com/store/apps/details?id=app.pantryandplate
- Trial: Start your 7-day free trial. Active subscription required after trial.

## One-Line Pitch

Pantry & Plate helps you turn pantry staples, saved recipes, and weekly plans into a focused grocery list.

## 25-Word Pitch

Pantry & Plate helps you plan meals from pantry staples, organize favorite recipes, and build grocery lists around what's missing after a 7-day trial.

## 50-Word Pitch

Pantry & Plate connects pantry tracking, My Cookbook, meal planning, and smart grocery lists so you can plan around what is already in your kitchen. Save recipes, build a weekly plan, review missing ingredients, and optionally launch connected shopping. Start your 7-day free trial; subscription required after trial.

## 100-Word Pitch

Pantry & Plate is a household meal-planning app built around the food and recipes you already have. Track pantry items, save and organize recipes in My Cookbook, plan meals for the week, and turn missing ingredients into a smarter grocery list. Optional connected shopping can help with handoff when you choose to use it, and AI-assisted recipe and image tools are available for user-started recipe workflows. Pantry & Plate is now available on the App Store and Google Play. Start your 7-day free trial; an active subscription is required after trial.

## Website Hero Copy Options

### Option 1

Headline: Cook tonight with what you already have.

Supporting copy: Pantry & Plate connects your pantry, recipes, weekly meal plan, and shopping list so you can plan dinner from your kitchen first.

CTA: Download on the App Store

Secondary: Start your 7-day free trial. Also available on Google Play.

### Option 2

Headline: Know what you have. Plan what you'll cook. Shop what's missing.

Supporting copy: Save household recipes, keep pantry staples visible, and build grocery lists around the actual gaps in your weekly plan.

CTA: Download on the App Store

Secondary: Active subscription required after trial.

### Option 3

Headline: A weekly meal plan that starts in your pantry.

Supporting copy: Pantry & Plate brings pantry tracking, My Cookbook, meal planning, and grocery lists into one connected household workflow.

CTA: Download on the App Store

Secondary: Start your 7-day free trial.

## Email Announcement

Subject: Pantry & Plate is now available on the App Store and Google Play

Hi,

Pantry & Plate is now live on the App Store and Google Play.

It is built for the weekly loop so many households repeat: check what is in the kitchen, choose recipes you actually want to cook, plan the week, and shop for what is missing.

With Pantry & Plate you can:

- Track pantry items and staples.
- Save and organize recipes in My Cookbook.
- Build weekly meal plans.
- Turn planned meals into smarter grocery lists.
- Use optional connected shopping when you choose.
- Use AI-assisted recipe and image tools for user-started recipe workflows.

Start your 7-day free trial:

- App Store: https://apps.apple.com/us/app/pantry-plate/id6761082174
- Google Play: https://play.google.com/store/apps/details?id=app.pantryandplate

An active subscription is required after trial.

Thanks for being part of the launch,
Pantry & Plate

## Founder Launch Note

I built Pantry & Plate because meal planning, pantry tracking, recipe saving, and grocery lists kept living in separate places.

The app brings those pieces into one household workflow: know what is already in your kitchen, plan what you will cook, and build a grocery list around what is missing. It also includes My Cookbook, optional connected shopping, and AI-assisted recipe and image tools for user-started recipe workflows.

Pantry & Plate is now available on the App Store and Google Play with a 7-day free trial. An active subscription is required after trial.

Download on the App Store:
https://apps.apple.com/us/app/pantry-plate/id6761082174

Get it on Google Play:
https://play.google.com/store/apps/details?id=app.pantryandplate

## Press Blurb

Pantry & Plate is a mobile app for household meal planning from the pantry outward. The app helps users track pantry items, save and organize recipes in My Cookbook, plan meals, and generate grocery lists around missing ingredients. Optional connected shopping and AI-assisted recipe and image tools are available for user-started workflows. Pantry & Plate is now available on the App Store and Google Play with a 7-day free trial; subscription required after trial.

## FAQ

### Where can I download Pantry & Plate?

Pantry & Plate is available on the App Store:
https://apps.apple.com/us/app/pantry-plate/id6761082174

It is also available on Google Play:
https://play.google.com/store/apps/details?id=app.pantryandplate

### Is Pantry & Plate free?

Eligible new users can start with a 7-day free trial. An active subscription is required after trial.

### What subscriptions are offered?

Pantry & Plate offers Premium Monthly and Premium Annual subscription options.

### What does Pantry & Plate help with?

It helps with pantry tracking, recipe saving and organization, My Cookbook, meal planning, smart grocery lists, household-friendly organization, optional connected shopping, and AI-assisted recipe and image tools.

### Does Pantry & Plate guarantee savings?

No. Pantry & Plate is designed to help you plan around what you have and what is missing, but it does not guarantee savings.

### Does Pantry & Plate provide medical, allergy, calorie, or weight-loss guidance?

No. Pantry & Plate is not a medical, allergy-safety, calorie-tracking, or weight-loss planning service.

### Does connected shopping mean checkout happens inside Pantry & Plate?

No. Optional connected shopping may help with handoff when you choose to use it, but checkout and order handling are managed by the connected grocery provider.

### How do I manage or cancel a subscription?

Manage or cancel subscriptions through the applicable App Store or Google Play account settings. Deleting a Pantry & Plate account does not automatically cancel a store subscription.
